...
- **Made Buttons Move:** At the end of each round, the made buttons move, according to some set pattern only the DM knows. Nothing happens if a made button ends its movement in the same space as a character.
- **Failed Checks Make Made Buttons:** Whenever a character fails a Check or loses a Contest while resolving a Focal Action or Side Action, the "alertness" of the "watchful eye" increases, represented by an additional made buttons being added to the number line.
